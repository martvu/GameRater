...
const MONGODB = 'mongodb://it2810-48.idi.ntnu.no:27017'
  ...
  const resolvers = {
    ...
    Mutation: {
      async createReview(_, { reviewInput : { author, title, content, rating }}) {
        const res = await new Review({author, title, content, rating}).save();
        return res._id;
      },
      async updateReview(_, { ID, reviewInput: { author, title, content, rating}}) {
        await Review.updateOne({_id: ID}, { $set: {author, title, content, rating}});
        return ID;
      },
